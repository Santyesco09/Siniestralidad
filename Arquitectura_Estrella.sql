...
/*
DROP TABLE FORMULARIO_ACCIDENTE CASCADE CONSTRAINTS; 
DROP TABLE VM_ACC_ACTOR_VIAL CASCADE CONSTRAINTS;
DROP TABLE VM_ACC_CAUSA CASCADE CONSTRAINTS;
DROP TABLE VM_ACC_VEHICULO CASCADE CONSTRAINTS;
DROP TABLE VM_ACC_VIA CASCADE CONSTRAINTS;

DELETE FROM FACT_VIAJE;
COMMIT;

-- Vaciar las tablas para evitar registros duplicados antes de volver a cargar con Python
TRUNCATE TABLE FORMULARIO_ACCIDENTE;
TRUNCATE TABLE VM_ACC_ACTOR_VIAL;
TRUNCATE TABLE VM_ACC_CAUSA;
TRUNCATE TABLE VM_ACC_VEHICULO;
TRUNCATE TABLE VM_ACC_VIA;
...
CREATE TABLE FORMULARIO_ACCIDENTE (
    ...
    CONSTRAINT FK_ESTRELLA_ACTOR FOREIGN KEY (ID_ACCIDENTADO) REFERENCES VM_ACC_ACTOR_VIAL(ID_ACCIDENTADO),
    CONSTRAINT FK_ESTRELLA_CAUSA FOREIGN KEY (ID_CAUSA) REFERENCES VM_ACC_CAUSA(ID_CAUSA),
    CONSTRAINT FK_ESTRELLA_VEHICULO FOREIGN KEY (ID_PLACA) REFERENCES VM_ACC_VEHICULO(ID_PLACA),
    CONSTRAINT FK_ESTRELLA_VIA FOREIGN KEY (ID_VIA) REFERENCES VM_ACC_VIA(ID_VIA)
);
